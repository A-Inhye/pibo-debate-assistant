"""
basic_server_pibo_design.py - 파이보 프로젝트용 커스텀 디자인 WhisperLiveKit 서버

이 모듈은 사용자 제안 디자인을 적용한 파이보 토론 지원 시스템 서버입니다.
FastAPI와 WebSocket을 사용하여 실시간 음성 전사 서비스를 제공합니다.

주요 엔드포인트:
    - GET /: 커스텀 디자인 웹 UI HTML 반환
...
사용법:
    ...
    # 또는 uvicorn으로 실행
    $ uvicorn whisperlivekit.basic_server_pibo_design:app --host 0.0.0.0 --port 8000
...
def main():
    ...
    # Uvicorn 기본 설정
    uvicorn_kwargs = {
        "app": "whisperlivekit.basic_server_pibo_design:app",  # 앱 경로 (문자열)
