"""
basic_server_pibo.py - 파이보 프로젝트용 WhisperLiveKit FastAPI WebSocket 서버
...
사용법:
    # Python 모듈로 실행 (권장)
    $ python -m whisperlivekit.basic_server_pibo --model large-v3 --language ko --enable-diarization

    # 또는 uvicorn으로 실행
    $ uvicorn whisperlivekit.basic_server_pibo:app --host 0.0.0.0 --port 8000
...
def main():
    ...
    # Uvicorn 기본 설정
    uvicorn_kwargs = {
        "app": "whisperlivekit.basic_server_pibo:app",  # 앱 경로 (문자열)
